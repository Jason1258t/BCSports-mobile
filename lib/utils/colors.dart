import 'package:flutter/material.dart';

List<Color> userColors = [
  Colors.red,
  Colors.blueAccent,
  Colors.green,
  Colors.pink,
  Colors.purple
];

abstract class AppColors {
  static Color black_090723 = const Color(0xff090723);
  static Color black_222232 = const Color(0xff222232);
  static Color black_101119 = const Color(0xff101119);
  static Color black_081131 = const Color(0xff081131);
  static Color black_252525 = const Color(0xff252525);
  static Color black_1A1A1A = const Color(0xff1A1A1A);
  static Color black_262627 = const Color(0xff262627);
  static Color black = Colors.black;
  static Color black_s2new_1A1A1A = const Color(0xff1A1A1A);
  static Color black_3A3A3A = Color(0xff3A3A3A);

  static Color brown_6C6226 = const Color(0xff6C6226);

  static Color yellow_F3D523 = const Color(0xffF3D523);

  static Color white = Colors.white;
  static Color white_F4F4F4 = const Color(0xffF4F4F4);
  static Color white_FFF7C7 = const Color(0xffFFF7C7);

  static Color purpure_F4F4F4 = const Color(0xffE7EBFd);

  static Color grey_B4B4B4 = const Color(0xffB4B4B4);
  static Color grey_B3B3B3 = const Color(0xFFB3B3B3);
  static Color grey_2F2F2F = const Color(0xFF2F2F2F);
  static Color grey_d9d9d9 = const Color(0xFFd9d9d9);
  static Color grey_393939 = const Color(0xFF393939);

  static Color primary = const Color(0xffF3D523);
  static Color background = Colors.black;
  static Color blueLink = const Color(0xFF2E8AF6);
}
