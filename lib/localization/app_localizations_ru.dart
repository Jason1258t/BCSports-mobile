import 'app_localizations.dart';

/// The translations for Russian (`ru`).
class AppLocalizationsRu extends AppLocalizations {
  AppLocalizationsRu([String locale = 'ru']) : super(locale);

  @override
  String get about => 'О игроке';

  @override
  String get academy => 'Академия';

  @override
  String get agree_terms => 'Я принимаю условия пользовательского соглашения';

  @override
  String get all_collection => 'Все коллекции';

  @override
  String get already_have_acc => 'Уже есть аккаунт?';

  @override
  String get ar => 'AR';

  @override
  String get ar_visual => 'AR визуализация Бразильской академии';

  @override
  String get assists => 'Передачи';

  @override
  String get available_bill => 'Доступный баланс';

  @override
  String get ball_recovery => 'Возврат владения';

  @override
  String get basketball => 'Баскетбол';

  @override
  String get be_first_to_write_a_message => 'Будьте первым, кто напишет сообщение';

  @override
  String get birth => 'Дата рождения';

  @override
  String get born => 'Родился';

  @override
  String get buy => 'Купить';

  @override
  String get cancel_lot => 'Отменить лот';

  @override
  String get cansel_sale => 'Вы уверены, что хотите отменить продажу?';

  @override
  String get capture_matrics => 'Захват показателей из футбольных матчей и физических показателей игроков нашими партнерами.';

  @override
  String get change_lang => 'Изменить язык';

  @override
  String get change_password => 'Изменить пароль';

  @override
  String get choose_play => 'Выбери, сыграй, победи';

  @override
  String get choose_this => 'Выбрать';

  @override
  String get citizen => 'Гражданство';

  @override
  String get club_name => 'Клуб';

  @override
  String get collection_stats => 'Сбор статистики игрока';

  @override
  String get comments => 'КОММЕНТАРИИ';

  @override
  String get country => 'Страна';

  @override
  String get create => 'Создать';

  @override
  String get delete => 'Удалить';

  @override
  String get delete_fail => 'Удалить сбой, попробуйте еще раз позже';

  @override
  String get democratize_sports => 'Демократизировать спортивные инвестиции, предоставляя фанатам возможность напрямую поддерживать спортсменов, предлагая спортсменам из всех слоев равный доступ к возможностям в своей карьере.';

  @override
  String get description => 'Описание';

  @override
  String get details => 'Подробности';

  @override
  String get discard => 'Отмена';

  @override
  String get dribbling_siffered => 'Дриблинг';

  @override
  String get duel_air => 'Дуэли в воздухе';

  @override
  String get duel_lost => 'Дуэлей проиграно';

  @override
  String get duel_tackle => 'Дуэль-такл';

  @override
  String get edit_profile => 'Редактировать профиль';

  @override
  String get email => 'Электронная почта';

  @override
  String get email_address => 'Адрес электронной почты';

  @override
  String get enter_registered_email => 'Пожалуйста, введите свой зарегистрированный адрес электронной почты.';

  @override
  String get exclusive_colls => 'Эксклюзивные цифровые колленкции';

  @override
  String get explore_more => 'Подробнее';

  @override
  String get fast_player_good_dribbling => 'Быстрого игрока с хорошим дриблингом и прохождением. Хорошо смотрится в атаке, но есть некоторая работа в защите.';

  @override
  String get favourite => 'Лайков';

  @override
  String get favs => 'Понравившиеся';

  @override
  String get feel_professional => 'Почувствуй себя проффесиональным гонщиком';

  @override
  String get fincher_best => 'Fincher - лучший и самый популярный веб -сайт для продажи, а также ваше искусство и коллекции в очень легком и безжалостном процессе.';

  @override
  String get fincher_over => 'Финчер - это чрезмерная сеть многих взаимосвязанных приложений и услуг для создания экосистемы децентрализованного будущего.';

  @override
  String get first_nft => 'Первые NFT, которые зависят не от рынка, а от упехов футболистов.';

  @override
  String get foot => 'Нога';

  @override
  String get footbag => 'Footbag';

  @override
  String get forgot_password => 'Забыли пароль';

  @override
  String get forgot_your_password => 'Забыли пароль?';

  @override
  String get fouls => 'Фолы';

  @override
  String get go_ar => 'Посмотреть в AR';

  @override
  String get goals => 'Голы';

  @override
  String get height => 'Высота';

  @override
  String get hide => 'скрыть';

  @override
  String get hit_the_max => 'Набей максимальное количество раз';

  @override
  String get killer_passes => 'Решающие передачи';

  @override
  String get language => 'Язык';

  @override
  String get likes => 'Лайки';

  @override
  String get log_out => 'Выйти';

  @override
  String get make_selfie => 'Сделай селфи или видео с любимым спортсменом, с помощью AR';

  @override
  String get market => 'Маркет';

  @override
  String get message => 'Сообщение';

  @override
  String get messages => 'Сообщения';

  @override
  String get mind => 'Что у тебя на уме?';

  @override
  String get mini_games => 'Мини-игры';

  @override
  String get minuts_played => 'Сыграно мин.';

  @override
  String get more => 'ещё';

  @override
  String get moto_pg => 'MotoPG';

  @override
  String get my_active_lots => 'Мои активные лоты';

  @override
  String get my_players => 'Мои игроки';

  @override
  String get name => 'Имя';

  @override
  String get next => 'Следующий';

  @override
  String get nft => 'NFT';

  @override
  String get nft_will_removed => 'NFT будет удален с рынка и возвращен в ваш инвентарь.';

  @override
  String get no => 'Нет';

  @override
  String get no_acc => 'У вас нет учетной записи?';

  @override
  String get no_favs => 'Похоже, у вас нет понравившихся';

  @override
  String get no_lots => 'Похоже, у вас нет активных лотов';

  @override
  String get no_post => 'Пока еще нет';

  @override
  String get password => 'Пароль';

  @override
  String get penalties => 'Пенальти';

  @override
  String get photo => 'Фото';

  @override
  String get player => 'Игрок';

  @override
  String get player_details => 'Детали игрока';

  @override
  String get position => 'Позиция';

  @override
  String get post => 'Пост';

  @override
  String get posts => 'Посты';

  @override
  String get profile => 'Профиль';

  @override
  String get project_mission => 'Проектная миссия';

  @override
  String get publish => 'Опубликовать';

  @override
  String get put_up_for_sale => 'Выставлен на продажу';

  @override
  String get quantity_matches => 'Кол-во матчей';

  @override
  String get replanish => 'Пополнить';

  @override
  String get save => 'Сохранить';

  @override
  String get sell => 'Продать';

  @override
  String get send => 'Отправить';

  @override
  String get send_recovery => 'Отправить почту для восстановления';

  @override
  String get set_price => 'Установить цену';

  @override
  String get settings => 'Настройки';

  @override
  String get show_aim => 'Покажи свою меткость и точность, забей трехочковый';

  @override
  String get sign_in => 'Войти';

  @override
  String get sign_up => 'Зарегистрироваться';

  @override
  String get skip => 'Пропустить';

  @override
  String get start => 'Начать';

  @override
  String get successful_return => 'Успешно, можете вернуться';

  @override
  String get swap => 'Обменять';

  @override
  String get talanted_individuals => 'Талантливые люди не могли стать успешными спортсменами из -за ограниченных ресурсов и должны были отказаться от своих мечтаний о стабильной жизни.';

  @override
  String get trick_goalkeeper => 'Обхитри вратаря и попади в ворота';

  @override
  String get trust_way => 'Самый надежный способ';

  @override
  String get type_comment => 'Написать коммент..';

  @override
  String get unlock_feature => 'Разблокируй будущее';

  @override
  String get username => 'Имя пользователя';

  @override
  String get views => 'Просмотры';

  @override
  String get wallet => 'Кошелек';

  @override
  String get we_spot_talents => 'Мы поддерживаем таланты';

  @override
  String get weight => 'Вес';

  @override
  String get yes => 'Да';

  @override
  String get you_dont_nfts => 'Извините, у вас нет NFT.';

  @override
  String get you_dont_posts => 'Изините, у вас нет постов.';

  @override
  String get stats => 'Статистика';

  @override
  String get chats => 'Чаты';

  @override
  String get home => 'Главная';

  @override
  String get add_nft => 'Добавить NFT';

  @override
  String get crop => 'Кадрирование';

  @override
  String get choose_photo => 'Выбрать фото';

  @override
  String get report_sended => 'Жалоба отправлена';

  @override
  String get try_again => 'Попробуйте снова';

  @override
  String get spam => 'Спам';

  @override
  String get violence => 'Насилие';

  @override
  String get child_abuse => 'Абъюз детей';

  @override
  String get porn => 'Порнография';

  @override
  String get copy => 'Кража';

  @override
  String get drugs => 'Наркотики';

  @override
  String get personal_details => 'Персональные данные';

  @override
  String get other => 'Другое';
}
